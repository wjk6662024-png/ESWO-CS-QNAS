...
def train_qnn(chromosome, generation, indiv_num, EPOCHS):
    ...
    cfg.QEA.train_times=cfg.QEA.train_times+1#计算需要训练的次数
    logger.info('Eval accuracy : %s', str(accuracy))
    if accuracy>=cfg.QEA.fitness[indiv_num]:
      ...
      os.system("mkdir -p ./weights/gen_{}/ind_{}acc_{}%".format(generation, indiv_num, np.round(cfg.QEA.fitness[indiv_num], 10) * 100))
      dir_init = "./weights/gen_{}/ind_{}*".format(generation-1, int(indiv_num))
      ...
      os.system("cp "+dir_init+"/model.arch "+dir_init+"/init.ckpt "+dir_init+"/best.ckpt "+dir_init+"/latest.ckpt "
              "./weights/gen_{}/ind_{}acc_{}%".format(generation, indiv_num, np.round(cfg.QEA.fitness[indiv_num], 10) * 100))
    del model, train_dataset, eval_dataset, model_eval
    return accuracy,"./weights/gen_{}/ind_{}acc_{}%".format(generation, indiv_num, np.round(accuracy, 10) * 100)
...
def train_qnn_final(chromosome, generation, indiv_num, EPOCHS):
    ...
    #    dir_init = glob.glob(dir_init)[0]
    #else:
    dir_init = sorted(glob.glob(dir_init))[-1]
